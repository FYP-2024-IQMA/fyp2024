...
export async function getAllLesson(): Promise<Lesson[]> {
  ...
  if (error) {
    ...
    return data.map((lesson) => ({
      ...lesson,
      dateCreated: lesson.dateCreated ? new Date(lesson.dateCreated) : null,
    })) as Lesson[];
  }
}
...
export async function getLesson(
  ...
  if (error) {
    ...
    const lessonKeyTakeawayAudio = await videoService.formatVideoUrl(
      null,
      sectionID,
      unitID,
      lessonID,
