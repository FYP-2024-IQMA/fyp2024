...
export const createUserStoneProgress = async (req: Request, res: Response) => {
    const { userID, last_completed_stone_index } = req.body;

    try {
        const result = await userStoneProgressService.createUserStoneProgress(userID, last_completed_stone_index);
        res.status(201).json({
...
export const updateUserStoneProgress = async (req: Request, res: Response) => {
    ...
    try {
        const result = await userStoneProgressService.updateUserStoneProgress(userID, last_completed_stone_index);
        res.status(200).json({
            status: 200,
...
export const deleteUserStoneProgress = async (req: Request, res: Response) => {
    try {
        ...
        res.status(200).json({
            status: 200,
            statusText: "Progress Deleted Successfully",
        });
    } catch (error: any) {
        const errorResponse = handleError(error);
        if (errorResponse) {
